...
csvFileName="HCAJ18_1_heads.csv"
videoFile="D:\\Data\\UMichHealth\\HCAJ18\\HCAJ18_1.mp4"
...
writer.writerow(["Frame", "Person", "Left","Top","Right","Bottom"])
...
def getHeadBox(frame,person,personNumber):
    y, x, c = frame.shape
    ...
    xs=[nose_x,leye_x,reye_x,lear_x,rear_x]
    ys=[nose_y,leye_y,reye_y,lear_y,rear_y]
...
def getPeopleHeadBoxes(frame,keypoints, boundingBoxes):
    boxes=[]
    personIndex=-1
    size=frame.shape
    personNumber=-1
    for person in keypoints:
        ...
        if confidencePerson>0.2:
            ...
            headBox=getHeadBox(frame,person,personNumber)
            boxes.append(headBox)
...
def saveBox(box,frameNumber,person):
    #print([frameNumber, person, int(box[0]), int(box[1]), int(box[2]), int(box[3])])
    writer.writerow([frameNumber, person, int(box[0]), int(box[1]), int(box[2]), int(box[3])])
...
while cap.isOpened():
    ...
    if True:
        ...
        headBoxes = getPeopleHeadBoxes(frame, keypoints_with_scores, boundingBoxes)

        # Draw the face detection annotations on the image.
        person=0
        print(frameNumber)
        for headBox in headBoxes:
            cv2.rectangle(frame, (int(headBox[0]), int(headBox[1])), (int(headBox[2]), int(headBox[3])), (255, 0, 0), 4)
            saveBox(headBox,frameNumber,person)
            person=person+1
